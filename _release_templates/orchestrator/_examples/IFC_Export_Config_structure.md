...
которая лежит в `_examples/IFC_Export_Config_example/` внутри
orchestrator-пакета.

ExportIFC не использует эту примерную папку напрямую. Она нужна как
стартовый ориентир для first-run: по ней настраивают реальный рабочий
каталог IFC-конфигов и сверяют его структуру.

Этот документ нужен как короткая карта файловой раскладки IFC-конфигов
именно для release-пакета. Он не заменяет основной manual,
а помогает быстро понять, где лежит общий слой, где проектный слой
и как эта структура связывается с `_settings/settings.ini`
и `manage.xlsx`.

Путь к рабочему каталогу задаётся в `_settings/settings.ini`
через параметр `dir_export_config`.

В release-пакете этот файл лучше открывать уже после
`README.txt` и практического manual `../_docs/ExportIFC_manual_ru.docx`,
когда вы дошли именно до настройки IFC-конфигов.

Полный архитектурный разбор связки `settings.ini`, `manage.xlsx`,
`Layer_Mapping.txt`, `Export_Settings.json` и `Property_Mapping.txt`
находится уже в репозитории проекта `ExportIFCFromRevit-CSharp`
...
2. в `manage.xlsx` проверить, что колонка **C**
   ...
   а колонки **D** и **F** содержат именно имена файлов без каталогов;
...
Для работы именно с этим orchestrator-пакетом начните с файлов,
которые лежат рядом:

- `../README.txt` — что входит в пакет и как проходить first-run;
- `../_docs/ExportIFC_manual_ru.docx` —
  практический manual по first-run, ручному запуску и диагностике;
- `../_settings/README.md` — как читать `settings.ini`
  в release-сценарии;
- `_examples/IFC_Export_Config_structure_notes_ru.docx` —
  дополнительные русскоязычные пояснения к примеру.

Если нужен полный архитектурный и эксплуатационный разбор,
его нужно смотреть уже в репозитории проекта
